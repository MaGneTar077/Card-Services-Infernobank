...
# Log Group para Lambda (nombre idéntico al de la función)
resource "aws_cloudwatch_log_group" "lambda" {
  name              = "/aws/lambda/${var.lambda_name}"
  retention_in_days = var.retention_in_days
}

# Log Group para API Gateway (ajustado a tu API real)
...
resource "aws_api_gateway_method_settings" "card_purchase_stage_all" {
  rest_api_id = aws_api_gateway_rest_api.CardApi.id
  stage_name  = aws_api_gateway_stage.card_purchase_stage.stage_name
